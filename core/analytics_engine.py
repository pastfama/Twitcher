...
class AnalyticsEngine:
    ...
    def update_stream(
        ...
        # ------------------------------------------------
        # The ViewerTracker already computed `percent` (the % change
        # over its history window) from the SAME viewer_count stream
        # that feeds the LCD counter and the history graph.  We must
        # NOT overwrite it with SullyGoose's static `viewer_growth`
        # (a rarely-updated daily metric) — doing so froze the MOM
        # gauge at 0/50 because `percent` was either absent (no SG
        # cache) or static (SG cached).
        #
        # SullyGoose growth is used only as a secondary status hint
        # when the real-time tracker has no signal yet (warming up).
        with self._stream_lock:
            sully = analysis.get("sullygoose", {}) or {}

        if "percent" not in analysis or analysis.get("percent") is None:
            # No real-time signal yet — fall back to SG growth if present.
            if sully:
            ...
            # Real-time percent exists — keep it.  Only refine the
            # status label with SG growth if the tracker is neutral.
            if analysis.get("status") in (None, "", "stable", "Stable", "warming up"):
                if sully:
                    growth = sully.get("viewer_growth")
                    if growth is None:
                        growth = 0
                ...
                if stats:
                    ...
                    if self._on_analytics_updated:
                        ...
                        analysis = {
                            ...
                            "sullygoose": stats,
                            # Include the login this fetch was for so the UI can
                            # guard against stale cross-channel results.
                            "_fetch_login": login,
                        }
