...
progress meters, plus VIEWERS/LIVE/CHAT/RAID colored indicators.
"""
...
class SullyGoosePanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # RAID indicator
        self._raid_var = tk.StringVar(value="RAID: —")
        self._raid = ctk.CTkLabel(self, textvariable=self._raid_var,
                                  text_color=Theme.MUTED, font=font(10, "bold"))
        self._raid.pack(anchor="w", padx=12, pady=(6, 10))
    ...
    def update(self, state: dict) -> None:
        ...
        raid = state.get("raid")
        if raid:
            self._raid_var.set(f"RAID: {raid.get('from','')} → {raid.get('to','')}")
            self._raid.configure(text_color=Theme.GREEN)
        else:
            self._raid_var.set("RAID: —")
            self._raid.configure(text_color=Theme.MUTED)
