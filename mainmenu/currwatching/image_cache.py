...
class ImageCache:
    ...
    def load(self, label, url, size, placeholder="?"):
        ...
        # Track which URL this label currently expects so that if
        # set_stream is called again before this download finishes,
        # the stale result is discarded instead of overwriting the
        # newer image (Fixes Issue #2).
        label.setProperty("_watcher_expected_url", url)
    ...
    def _on_done(self, result, label, size, placeholder):
        ...
        # If the label has moved on to a different URL (e.g. set_stream
        # was called again while this download was in flight), discard
        # the result so we don't show a stale avatar (Fixes Issue #2).
        expected = label.property("_watcher_expected_url")
        if expected is not None and str(expected) != url:
            logger.debug(
                "Discarding stale image download for %s "
                "(label now expects %s)", url, expected
            )
            return
        ...
        if pixmap.loadFromData(data):
            ...
            # Cache the pixmap even if the label moved on — it may
            # be requested again later for a different stream.
